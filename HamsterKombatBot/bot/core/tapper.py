        ...
        while True:
            try:
                ...
                if time() - access_token_created_time >= 3600:
                    ...
                    logger.info(
                        f'{self.session_name} | Последний пассивный доход: <g>+{last_passive_earn:,}</g> | '
                        f'Заработок каждый час: <y>{earn_on_hour:,}</y>'
                    )
                            ...
                            if start_bonus_round <= datetime.now() < end_bonus_round:
                                ...
                                if not is_combo_accessible:
                                    logger.info(f"{self.session_name} | "
                                                f"<r>Ежедневная комбо не доступна</r>, вы можете приобрести только {possible_cards_count} из {need_cards_count} карт")

                                if balance < common_price:
                                    logger.info(f"{self.session_name} | "
                                                f"<r>Ежедневная комбо не доступна</r>, у вас недостаточно монет. Требуется <y>{common_price:,}</y> монет, но у вас <r>{balance:,}</r> монет")
                                ...
                                if common_price < bonus and balance > common_price and is_combo_accessible:
                                    for upgrade in available_combo_cards:
                                        ...
                                        logger.info(
                                            f'{self.session_name} | '
                                            f'Ожидание 5с перед улучшением <r>комбо</r> карты <e>{upgrade_id}</e>'
                                        )
                                        ...
                                        if status is True:
                                            ...
                                            logger.success(
                                                f'{self.session_name} | '
                                                f'Успешно улучшена <e>{upgrade_id}</e> по цене <r>{price:,}</r> до уровня <m>{level}</m> | '
                                                f'Заработок каждый час: <y>{earn_on_hour:,}</y> (<g>+{profit:,}</g>) | '
                                                f'Осталось монет: <e>{balance:,}</e>'
                                            )
                                    ...
                                    if status is True:
                                        logger.success(
                                            f'{self.session_name} | Успешно получена ежедневная комбо | '
                                            f'Бонус: <g>+{bonus:,}</g>'
                                        )
                    ...
                    if is_completed is False:
                        ...
                        if status is True:
                            logger.success(
                                f'{self.session_name} | Успешно получена ежедневная награда | '
                                f"Дни: <m>{days}</m> | Награда: {rewards[days - 1]['rewardCoins']} монет"
                            )
                    ...
                    if daily_cipher:
                        ...
                        if not is_claimed and cipher:
                            ...
                            if status is True:
                                logger.success(
                                    f'{self.session_name} | '
                                    f'Успешно расшифрован ежедневный шифр: <y>{decoded_cipher}</y> | '
                                    f'Бонус: <g>+{bonus:,}</g>'
                                )
                    ...
                    if not exchange_id:
                        ...
                        if status is True:
                            logger.success(
                                f'{self.session_name} | Успешно выбран обменник <y>Bybit</y>'
                            )
                ...
                logger.success(
                    f'{self.session_name} | Успешное нажатие! | '
                    f'Баланс: <c>{balance:,}</c> (<g>+{calc_taps:,}</g>) | Всего: <e>{total:,}</e>'
                )
                ...
                if active_turbo is False:
                    if settings.AUTO_UPGRADE is True:
                        for _ in range(settings.UPGRADES_COUNT):
                            ...
                            logger.info(
                                f'{self.session_name} | Ожидание 5с перед улучшением <e>{upgrade_id}</e>'
                            )
                            await asyncio.sleep(delay=5)
                    ...
                    if available_energy < settings.MIN_AVAILABLE_ENERGY:
                        ...
                        if (
                            ...
                            logger.info(
                                f'{self.session_name} | Ожидание 5с перед применением энергетического буста'
                            )
                            await asyncio.sleep(delay=5)
                            ...
                            if status is True:
                                logger.success(
                                    f'{self.session_name} | Успешно применен энергетический буст'
                                )
                        ...
                        logger.info(
                            f'{self.session_name} | Достигнут минимальный уровень энергии: <y>{available_energy}</y>'
                        )
                        logger.info(
                            f'{self.session_name} | Ожидание {random_sleep:,}с'
                        )
                ...
                logger.info(f'Ожидание {sleep_between_clicks}с')
...
async def run_tapper(tg_client: Client, proxy: str | None):
    try:
        ...
        logger.error(f'{tg_client.name} | Неверная сессия')
